...
const Dropdown = forwardRef<HTMLDivElement, DropdownProps>(
  ({ options, selectedOption, onOptionSelect, className, children, onEscape, ...props }, ref) => {
      ...
      if (focusedIndex === null) {
        ...
        if (event.key === 'ArrowDown') {
          ...
          setFocusedIndex((prevIndex) =>
            prevIndex !== null && prevIndex < options.length - 1 ? prevIndex + 1 : 0,
          );
        } else if (event.key === 'ArrowUp') {
          event.preventDefault();
          setFocusedIndex((prevIndex) =>
            prevIndex !== null && prevIndex > 0 ? prevIndex - 1 : options.length - 1,
          );
    ...
    useEffect(() => {
      if (internalRef.current && selectedOption !== undefined) {
        ...
        if (selectedIndex !== -1) {
          ...
          const selectedOptionElement = internalRef.current.querySelectorAll<HTMLElement>(
            '.' + styles.option,
          )[selectedIndex];
