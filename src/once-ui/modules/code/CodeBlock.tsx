  ...
  return (
    ...
    >
        ...
        >
          {codeInstances.length > 1 ? (
            <Flex>
              ...
              >
                <Button size="s" label={label} suffixIcon="chevronDown" variant="tertiary" />
              </DropdownWrapper>
            </Flex>
          ) : (
            <div />
          ...
          {copyButton && !compact && (
            <IconButton tooltip="Copy" variant="secondary" onClick={handleCopy} icon={copyIcon} />
          )}
        </Flex>
      )}
      {codePreview && (
      ...
      {codeInstances.length > 0 && (
        <Flex
          style={{
            borderTop: !compact && codePreview ? '1px solid var(--neutral-border-medium)' : 'none',
          }}
          fillWidth
          padding="8"
          position="relative"
